...
async function resolveUserEmail(userId) {
  ...
  try {
    ...
    console.error("Erreur resolveUserEmail :", error.message);
    return null;
  }
}
...
async function resolveAdminEmails() {
  ...
  if (process.env.ADMIN_EMAIL) {
    emails.add(process.env.ADMIN_EMAIL);
  }
  ...
  try {
    ...
    console.warn("⚠️ Résolution emails admins :", error.message);
  }
...
function emitNotification(io, notification) {
  ...
  if (notification.audience_role) {
    io.to(`role:${notification.audience_role}`).emit("notification:new", notification);
  }

  if (notification.audience_role === "admin") {
...
async function createNotification({
  ...
  if (!title || !message) {
    throw new Error("title et message sont requis.");
  }
  ...
  if (emailTo) {
    recipients = Array.isArray(emailTo) ? emailTo : [emailTo];
  } else if (userId) {
    const userEmail = await resolveUserEmail(userId);
    if (userEmail) recipients.push(userEmail);
  } else if (audienceRole === "admin") {
    recipients = await resolveAdminEmails();
  }

  recipients = [...new Set(recipients.filter(Boolean))];

  if (!recipients.length) {
    await NotificationModel.setEmailResult(notification.id, {
      sent: false,
      error: "Aucun destinataire email.",
    });
  ...
  const errors = results
    .map((result) => {
      ...
      if (!result.value?.sent) {
        return result.value?.error || "Email non envoyé.";
      }
